...
namespace Util{
	...
	*/
	std::string toString(int n){std::stringstream ss; ss << n; return ss.str();}
	...
	*/
	...
	std::string convertMat2String2(cv::Mat mat) {
		...
		base64::Encode(iss, oss);
		*/
		std::ostringstream oss(string(buf.begin(), buf.end()));
		return oss.str();
	}
	/*
	std::string GetCurrentTimeString(){
		...
		char strDate[128];
		sprintf_s( strDate, "%4d-%02d-%02d-%02d-%02d-%02d", 
			local.tm_year + 1900,
			local.tm_mon + 1,
			local.tm_mday,
			local.tm_hour,
